...
//
//  be aware that the destination EEPROM is overwritten.
//  so be sure to get the source and destination address right.
//  destination eeprom must be at least equal in size.
...
void setup()
{
  ...
  for (uint32_t addr = 0; addr < ee1.getDeviceSize(); addr += BUFFER_SIZE)
  {
    ...
    bool b1 = ee1.readBlock(addr, buffer, BUFFER_SIZE);
    if (b1 == false)
    {
      Serial.print("Read error at block: ");
      Serial.println(addr, HEX);
    }
    bool b2 = ee2.writeBlockVerify(addr, buffer, BUFFER_SIZE);
    if (b2 == false)
    {
